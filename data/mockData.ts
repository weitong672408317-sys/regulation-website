...
const fallbackCountries: CountryData[] = [
  ...
  trendsWarnings: {
    trendAnalysis: '核心判断：印尼目前没有明显信号显示将对传统烟草、HNB或电子烟实施全国性全面禁令。\\n\\n- 对非法卷烟和无税票产品持续高压执法，海关及消费税总局查处数量明显增加。\\n- 电子烟监管更关注流通端和非法产品，特别是含毒品电子烟液、违禁成分、BPOM流通监管、年龄限制、包装警示和广告限制。\\n- 消费税政策可能继续围绕非法市场治理调整，但是否新增税收层级、如何调整税率结构，仍存在政策讨论和争议。\\n- 整体看，印尼仍是烟草及新型烟草产品可准入且相对友好的市场，政策重点是把产品纳入合法资质、税务、包装和销售监管体系，而不是对相关品类进行全面封禁。\\n- 传统卷烟和HNB的重点风险在税票、NPPBKC、清关和包装；电子烟的重点风险在BPOM监管、成分和非法烟液；尼古丁袋/口含膜的重点风险在产品定性和主管机关口径不稳定。',
    redLines: [
